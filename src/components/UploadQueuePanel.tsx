...
export const UploadQueuePanel = ({ isOpen, onClose, onReselectFile }: UploadQueuePanelProps) => {
    ...
    if (draggedId && draggedId !== targetId) {
      ...
      if (draggedIndex > targetIndex) {
        ...
        for (let i = draggedIndex; i > targetIndex; i--) {
          reorderUpload(draggedId, 'up');
        }
      } else {
        // Move down
        for (let i = draggedIndex; i < targetIndex; i++) {
          reorderUpload(draggedId, 'down');
        }
      }
    }
    setDraggedId(null);
  ...
  return (
    <Card className="fixed bottom-4 right-4 w-[420px] shadow-2xl z-50 max-h-[80vh] flex flex-col">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {upload.status === 'paused' && !upload.file && (
                        <Button
                          ...
                          size="icon"
                          className="h-6 w-6"
                          onClick={() => onReselectFile(upload.id, upload.fileName)}
                          title="Reselect file to resume"
                        >
                      ...
                      {upload.status === 'paused' && upload.file && (
                        <Button
                          ...
                          size="icon"
                          className="h-6 w-6"
                          onClick={() => resumeUpload(upload.id, upload.file!)}
                        >
                      ...
                      {upload.status === 'error' && (
                        <Button
                          ...
                          size="icon"
                          className="h-6 w-6"
                          onClick={() => toggleDiagnostics(upload.id)}
                    ...
                    <div className="mt-2 p-2 bg-muted/50 rounded text-xs space-y-1">
                      <div className="flex justify-between items-center">
                        ...
                        <Button
                          ...
                          onClick={() => copyDiagnostics(upload)}
                        >
